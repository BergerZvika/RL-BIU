...
class RewardTracker:
    ...
    def reward(self, reward, frame, epsilon=None):
        ...
        if ts_diff > self.min_ts_diff:
            ...
            ### for graph plot
            episodes_graph.append(len(self.total_rewards))
            mean_reward_graph.append(mean_reward)

            if len(self.total_rewards) % 200 == 0 or len(self.total_rewards) % 500 == 0:
                plt.plot(episodes_graph,mean_reward_graph, "^", color='#EDB120')
                plt.ylabel("Mean Reward")
                plt.xlabel("Episodes")
                plt.title(f"Mean Reward Over {len(self.total_rewards)} Episodes")
                image = f"./images/{len(self.total_rewards)} Episodes.png"
                plt.savefig(image)
                # plt.show()
            ###
            sys.stdout.flush()
